...
/** @class
  ...
  `SC.DateTime` object, e.g.
      ...
      // London
      var d = moment({ hour: 5, timezone: 300 }); // -5 hours from UTC
      var e = moment({ hour: 10, timezone: 0 }); // same time, specified in UTC
...
/** @scope SC.DateTime.prototype */ {
  ...
  */
  format: function(fmt) {
    return this.constructor._format(fmt, this._ms, this.timezone);
  },

  /**
    Formats the receiver according ISO 8601 standard. It is equivalent to
    calling format with the `'%Y-%m-%dT%H:%M:%S%Z'` format string.

    @return {String} the formatted string
  */
  toISO8601: function(){
    return this.constructor._format(SC.DATETIME_ISO8601, this._ms, this.timezone);
  },
  ...
  /**
    ...
    `SC.DateTime` is designed, calling `SC.DateTime._format` would
...
/** @scope SC.DateTime */ {
  ...
  */
  ...
  /**
    ...
    @see SC.DateTime#format for a description of the format parameter
  ...
  */
  parse: function(str, fmt) {
    ...
    try {
      ...
      SC.Logger.log('momentFromString ' + e.toString());
      return null;
    }
  ...
  */
  ...
  /**
    @private
    @see SC.DateTime#_format
  */
  __format: function(part, start, timezone) {
    var hour, offset;
    ...
    //
    // This method is simply a helper for this._format() (one underscore);
    // this is only called from there, and _format() has already
  ...
  /**
    @private
    @see SC.DateTime#format
  */
  _format: function(format, start, timezone) {
    var that = this;
    ...
    return format.replace(/\%([aAbBcdDhHiIjmMpsSUWwxXyYZ\%])/g, function() {
      var v = that.__format.call(that, arguments, start, timezone);
      return v;
    });
  },
  ...
  SC.RecordAttribute.registerTransform(SC.DateTime, {
    ...
    */
    to: function(str, attr) {
      ...
      if (SC.none(str) || (str instanceof Date)) return moment(str.getTime());
      var format = get(attr, 'format');
      return moment(str, format ? format : SC.DateTime.recordFormat);
    },
    ...
    */
    from: function(dt, attr) {
      ...
      var format = get(attr, 'format');
      return dt.format(format ? format : SC.DateTime.recordFormat);
    }
  });
