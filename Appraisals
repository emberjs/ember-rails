appraise "rails31_ember_rc2" do
  gem 'rails', '~> 3.1'
  gem 'ember-source', "1.0.0.rc2.2"
  gem 'handlebars-source', '1.0.0.rc3'
end

appraise "rails31_ember_rc3" do
  gem 'rails', '~> 3.1'
  gem 'ember-source', "1.0.0.rc3.4"
  gem 'handlebars-source', '1.0.0.rc4'
end

appraise "rails31_ember_rc8" do
  gem 'rails', '~> 3.1'
  gem 'ember-source', "1.0.0.rc8"
  gem 'handlebars-source', '~>1.0.0'
end

appraise "rails31_ember_100" do
  gem 'rails', '~> 3.1'
  gem 'ember-source', '1.0.0'
  gem 'handlebars-source', '~> 1.0.0'
end

appraise "rails31_ember_110" do
  gem 'rails', '~> 3.1'
  gem 'ember-source', '1.1.0'
  gem 'handlebars-source', '~> 1.0.0'
end

appraise "rails32_ember_rc2" do
  gem 'rails', '~> 3.2'
  gem 'ember-source', "1.0.0.rc2.2"
  gem 'handlebars-source', '1.0.0.rc3'
end

appraise 'rails32_ember_rc3' do
  gem 'rails', '~> 3.2'
  gem 'ember-source', '1.0.0.rc3.4'
  gem 'handlebars-source', '1.0.0.rc4'
end

appraise 'rails32_ember_rc8' do
  gem 'rails', '~> 3.2'
  gem 'ember-source', '1.0.0.rc8'
  gem 'handlebars-source', '~>1.0.0'
end

appraise "rails32_ember_100" do
  gem 'rails', '~> 3.2'
  gem 'ember-source', '1.0.0'
  gem 'handlebars-source', '~> 1.0.0'
end

appraise "rails32_ember_110" do
  gem 'rails', '~> 3.2'
  gem 'ember-source', '1.1.0'
  gem 'handlebars-source', '~> 1.0.0'
end

appraise "rails4_ember_rc2" do
  gem 'rails', '~> 4.0.0'
  gem 'ember-source', "1.0.0.rc2.2"
  gem 'handlebars-source', '1.0.0.rc3'
end

appraise "rails4_ember_rc3" do
  gem 'rails', '~> 4.0.0'
  gem 'ember-source', "1.0.0.rc3"
  gem 'handlebars-source', '1.0.0.rc3'
end

appraise "rails4_ember_rc5" do
  gem 'rails', '~> 4.0.0'
  gem 'ember-source', "1.0.0.rc5"
  gem 'handlebars-source', '1.0.0.rc4'
end

appraise "rails4_ember_rc6" do
  gem 'rails', '~> 4.0.0'
  gem 'ember-source', "1.0.0.rc6"
  gem 'handlebars-source', '1.0.0.rc4'
end

appraise "rails4_ember_rc6.4" do
  gem 'rails', '~> 4.0.0'
  gem 'ember-source', "1.0.0.rc6.4"
  gem 'handlebars-source', '~> 1.0.0'
end

appraise "rails4_ember_rc7" do
  gem 'rails', '~> 4.0.0'
  gem 'ember-source', '1.0.0.rc7'
  gem 'handlebars-source', '~> 1.0.0'
end

appraise "rails4_ember_rc8" do
  gem 'rails', '~> 4.0.0'
  gem 'ember-source', '1.0.0.rc8'
  gem 'handlebars-source', '~> 1.0.0'
end

appraise "rails4_ember_100" do
  gem 'rails', '~> 4.0.0'
  gem 'ember-source', '1.0.0'
  gem 'handlebars-source', '~> 1.0.0'
end

appraise "rails4_ember_110" do
  gem 'rails', '~> 4.0.0'
  gem 'ember-source', '1.1.0'
  gem 'handlebars-source', '~> 1.0.0'
end
